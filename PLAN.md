...
| Landing page | `frontend/src/landing/` | 🟡 **Built, screenshot-verified, not deployed** — public marketing page at `/` on web only; the app lives on from `/home`. One SPA, one domain: the existing Vercel catch-all rewrite already serves both. Verified in headless Chrome (light + dark, hero, bento, protocol, sign-in, gated redirect). §7 #48–50 | — |
...
| 48 | **The web auth gate moved into the router; the landing page at `/` is public.** Native is unchanged | `_layout.tsx` rendered `<SignInScreen/>` in place of the whole tree for any signed-out user, on every platform. Correct for an installed app — someone who downloaded it has already been sold — but on the web it meant a visitor hit a password form before ever learning what Sahayak is. On web the router now always mounts, `/` and `/sign-in` are public, and `WebAuthGate` bounces every other route to sign-in. Landing and app therefore ship as **one SPA on one domain**; the `vercel.json` catch-all rewrite already serves it, so no second project or subdomain |
| 49 | The landing page is **DOM elements in a `.web.tsx`**, not React Native Web, with its CSS scoped under `.lp` | A scroll-driven marketing page (sticky nav, canvas particle field, CSS-grid bento, keyframe choreography) expressed through RN's layout model would cost real fidelity for portability that a page which never runs on a phone does not need. Metro resolves `.web.tsx`, so none of it reaches the native bundle. **Every rule is scoped under `.lp`** and injected as a `<style>` child living exactly as long as the component: the app *is* RN Web, so a bare `section`/`h2`/`*` reset would otherwise reach straight into the app tree. Dark tokens use the app's own palette (`#12100C`), not the standalone mockup's lighter charcoal — against the real canvas the mockup's aurora read as a saturated wash over the type, so its opacity is deliberately lower here |
| 50 | Document `<head>` is finished by a **post-export script**, not `app/+html.tsx` | `+html.tsx` is the documented way to own the shell but applies only to static rendering; web is pinned to `output: "single"` (§7 #36), where Expo emits a fixed template — verified by exporting with the file in place and getting Expo's default head back. Rather than change the output mode and the shape of the deploy, `scripts/finalize-web-head.mjs` runs after export (wired into the Vercel `buildCommand`) and adds the title, description, OG tags, `theme-color`, and a no-flash dark script. Idempotent, and it fails loudly if Expo's template stops matching. Per-page titles are separate — set at runtime from the route by `WebPageTitle`, because the web tab layout renders a `Slot`, not `Tabs`, so per-screen `title` options never reach it |
| 51 | The landing page **links out** to the merchant dashboard at `https://sahayak-merchant-dashboard.up.railway.app/` rather than hosting a copy | The dashboard deploys as one service where FastAPI serves its own Vite build — its Dockerfile says why: same origin means the frontend keeps calling a relative `/api` and CORS never applies. Re-hosting that build under `/dashboard` on Vercel was briefly done and then removed: it produced a **second, diverging copy with no backend**, permanently showing "the server sent a response we could not read", and reproducing the same-origin session would have required proxying `/api` back out to Railway. Leaving the site is the cheaper and more honest boundary — it is a different product for a different audience (merchants, not customers). `Onboard Business` therefore sits with the nav actions, not among the section anchors |
| 52 | **A destructive capability now triages against the merchant's published policy before it runs.** The confirm gate alone approved every refund | `refund` asked one yes/no and executed — it never established *why*, never read the return policy, and approved a customer outside the window as readily as one inside it. Triage folds into the **existing** `prepare` prompt rather than a new node, so a gated turn still costs one reasoning call and #16's 58 s failure mode stays excluded. The runtime supplies only the procedure — ask, count, verify, stop — and the manifest supplies every question and every word of a refusal. Verified: the same opening sentence produces different questions for `ravi-electronics` and `sri-pharma`, with no runtime change |
| 53 | **A refusal must quote the business verbatim, or it is not a refusal** | Nothing stopped the model inventing a policy and refusing a customer in the merchant's voice, which is worse than approving too freely because it sounds authoritative. `_quotes_the_policy()` requires `policy_basis` to appear in `manifest.knowledge_text()` after whitespace/case normalisation; a quote that isn't there downgrades to `unknown` and escalates. Live-verified both ways — a real clause passes, a fabricated one is rejected and logged `triage.unsupported_denial` |
| 54 | **Facts are looked up, not asked for.** Before triaging, the runtime runs the manifest's own read-only capability whose inputs are already satisfied | The first working build asked "when was your order delivered?" about an order the store can see — the difference between an agent and a form. `_look_up_evidence()` finds *a read-only capability it can already satisfy* (for these merchants, `track_order` ⊆ `refund`) and passes the result in as established fact. Business-generic: the runtime has no idea what an order or a delivery date is. `_map_order` now derives `delivered_on`/`days_since_delivery` from the real Shopify fulfilment, so the field §5's own example rule always assumed finally exists |
| 55 | **Rules fail closed on a write, and eligibility is evaluated before the action** | `act` logged `RuleError` and `continue`d, so a rule naming a field the connector doesn't return silently *permitted* the action — the exact shape that refunds an ineligible customer. A rule the runtime cannot evaluate is not a rule that passed: on a `confirm` capability it now escalates, while read-only capabilities still continue so a broken rule can't take order tracking down. Rules that don't mention `result` are asking "may we?", not "what happened?", and now run **before** `executor.execute` — deciding eligibility after the endpoint call means the refund is already away. Note `normalize.py` still emits `rules: []`, so this engine remains inert until a manifest publishes one; the prose-policy path above is what actually gates today |
| 56 | A policy refusal ends with the manifest's own `escalation.message` | A refusal that just says no is a job abandoned (§1). The runtime contributes no contact, no team and no wording — it repeats what the merchant published, so the customer leaves with a route (`grievance@ravielectronics.in`) rather than a dead end |
