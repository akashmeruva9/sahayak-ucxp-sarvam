...
export const api = {
  ...
  /** One spoken recording -> { ok, fields, heard, language, error }. */
  voiceOnboard: (blob, filename = 'speech.webm') => {
    const form = new FormData();
    form.append('audio', blob, filename);
    return request('/voice-onboard', {
      method: 'POST',
      timeoutMs: 110000, // must exceed the server's own 90s budget
      body: form,
      // Deliberately empty: FormData sets its own Content-Type, and the
      // multipart boundary is generated by the browser. Leaving our JSON
      // default in place makes the body unparseable on the server.
      headers: {},
    });
  },
